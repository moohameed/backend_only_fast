...
export const getAddressesUser = async (req, res = response ) => {
    ...
    try {
        ...
        const addressesdb = await pool.query('SELECT * FROM addresses WHERE persona_id = ?', [req.uid]);
...
export const addStreetAddress = async ( req, res = response ) => {
    ...
    try {
        ...
        const { street, Latitude, Longitude } = req.body;

        await pool.query('INSERT INTO addresses (street, Latitude, Longitude, persona_id) VALUE (?,?,?,?)', [ street, Latitude, Longitude, req.uid ]);
        console.log('INSERT INTO addresses (street, Latitude, Longitude, persona_id) VALUE (?,?,?,?)', [ street, Latitude, Longitude, req.uid ])
        res.json({
            resp: true,
...
export const getAddressOne = async (req, res = response) => {
    ...
    try {
        ...
        const addressdb = await pool.query('SELECT * FROM addresses WHERE persona_id = ? and default_address = 1', [ req.uid ]);
...
export const getUserNotificationToken = async (req, res = response ) => {

    try {

        const token = await pool.query('SELECT notification_token FROM users WHERE id = ?', req.params.userID);

        return res.json({
            resp: true,
            msg: "User Token",
            token: token[0]["notification_token"]
        });
