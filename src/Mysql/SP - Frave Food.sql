...
DELIMITER //
CREATE PROCEDURE SP_GET_RESTAURANTS()
BEGIN
SELECT rest.id, rest.name, rest.phone, rest.address, ir.picture FROM restaurants rest INNER JOIN imagerestaurant ir ON rest.id = ir.restaurant_id INNER JOIN ( SELECT restaurant_id, MIN(id) AS id_image FROM imagerestaurant GROUP BY restaurant_id) p3 ON ir.restaurant_id = p3.restaurant_id AND ir.id = p3.id_image LIMIT 10;
END//
...
INSERT INTO imageProduct (picture, product_id) VALUES 
	...
	SELECT ingre.label, resto.name, typeing.label
	FROM ingredients ingre, restaurant_ingredients resting, restaurants resto, ingredient_type typeing
	WHERE ingre.id = resting.ingredient_id and resto.id = resting.restaurant_id and typeing.id = ingre.type_id;
