...
-- Procedures
--
CREATE  PROCEDURE `SP_ADD_CATEGORY` (IN `category` VARCHAR(50), IN `description` VARCHAR(100))   BEGIN
	INSERT INTO categories (category, description) VALUE (category, description);
END$$

CREATE  PROCEDURE `SP_ALL_DELIVERYS` ()   BEGIN
...
CREATE  PROCEDURE `SP_ALL_ORDERS_STATUS` (IN `statuss` VARCHAR(30))   BEGIN
...
CREATE  PROCEDURE `SP_GET_PRODUCTS_CATEGORY` (IN `idRestaurant` INT, IN `idCategory` INT)   BEGIN
...
CREATE  PROCEDURE `SP_GET_PRODUCTS_TOP` (IN `idRestaurant` INT)   BEGIN
	SELECT pro.id, pro.restaurant_id, pro.nameProduct, pro.description, pro.price, pro.status, ip.picture, pro.restaurant_id, c.category, c.id AS category_id, c.icon as icon FROM products pro
	INNER JOIN categories c ON pro.category_id = c.id
	INNER JOIN imageProduct ip ON pro.id = ip.product_id
	INNER JOIN ( SELECT product_id, MIN(id) AS id_image FROM imageProduct GROUP BY product_id) p3 ON ip.product_id = p3.product_id AND ip.id = p3.id_image
...
CREATE  PROCEDURE `SP_GET_RESTAURANTS` (IN `clientId` INT)   BEGIN
...
CREATE  PROCEDURE `SP_GET_Restaurants_Admin` ()   BEGIN
...
CREATE  PROCEDURE `SP_GET_RESTAURANTS_CAT` (IN `categoryId` INT, IN `clientId` INT)   BEGIN
...
CREATE  PROCEDURE `SP_GET_RESTAURANTS_FAV` (IN `clientId` INT)   BEGIN
...
CREATE  PROCEDURE `SP_LIST_PRODUCTS_ADMIN` ()   BEGIN
...
CREATE  PROCEDURE `SP_LOGIN` (IN `email` VARCHAR(100))   BEGIN
...
CREATE  PROCEDURE `SP_ORDERS_BY_DELIVERY` (IN `ID` INT, IN `statuss` VARCHAR(30))   BEGIN
...
CREATE  PROCEDURE `SP_ORDERS_FOR_CLIENT` (IN `ID` INT)   BEGIN
...
CREATE  PROCEDURE `SP_ORDER_DETAILS` (IN `IDORDER` INT)   BEGIN
...
CREATE  PROCEDURE `SP_ORDER_DETAILSS` (IN `IDORDER` INT)   BEGIN
	SELECT od.id, od.order_id, od.product_id, p.nameProduct, ip.picture, od.quantity, od.price AS total
	FROM orderdetails od
	INNER JOIN products p ON od.product_id = p.id
	INNER JOIN imageProduct ip ON p.id = ip.product_id
...
CREATE  PROCEDURE `SP_REGISTER` (IN `firstName` VARCHAR(50), IN `lastName` VARCHAR(50), IN `phone` VARCHAR(11), IN `image` VARCHAR(250), IN `email` VARCHAR(100), IN `pass` VARCHAR(100), IN `rol` INT, IN `nToken` VARCHAR(255))   BEGIN
...
CREATE  PROCEDURE `SP_RENEWTOKENLOGIN` (IN `uid` INT)   BEGIN
...
CREATE  PROCEDURE `SP_SEARCH_FOR_CATEGORY` (IN `IDCATEGORY` INT)   BEGIN
	SELECT pro.id, pro.nameProduct, pro.description, pro.price, pro.status, ip.picture, c.category, c.id AS category_id FROM products pro
	INNER JOIN categories c ON pro.category_id = c.id
	INNER JOIN imageProduct ip ON pro.id = ip.product_id
	INNER JOIN ( SELECT product_id, MIN(id) AS id_image FROM imageProduct GROUP BY product_id) p3 ON ip.product_id = p3.product_id AND ip.id = p3.id_image
...
CREATE  PROCEDURE `SP_SEARCH_PRODUCT` (IN `nameProduct` VARCHAR(100))   BEGIN
	SELECT pro.id, pro.nameProduct, pro.description, pro.price, pro.status, ip.picture, c.category, c.id AS category_id FROM products pro
	INNER JOIN categories c ON pro.category_id = c.id
	INNER JOIN imageProduct ip ON pro.id = ip.product_id
	INNER JOIN ( SELECT product_id, MIN(id) AS id_image FROM imageProduct GROUP BY product_id) p3 ON ip.product_id = p3.product_id AND ip.id = p3.id_image
...
CREATE  PROCEDURE `SP_UPDATE_PROFILE` (IN `ID` INT, IN `firstName` VARCHAR(50), IN `lastName` VARCHAR(50), IN `phone` VARCHAR(11))   BEGIN
	UPDATE person
...
CREATE  PROCEDURE `SP_USER_BY_ID` (IN `ID` INT)   BEGIN
...
CREATE  PROCEDURE `SP_USER_UPDATED` (IN `ID` INT)   BEGIN
...
CREATE TABLE `products` (
  ...
  `picture` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT 'without-image.jpg',
...
(1, 'Frave', 'frave@frave.com', '$2b$10$loiXWqS2XD3Xa5rPwShlwu9tcX3QQYwMHtXNVU0yIrIDQiHigybUC', 1, NULL, 1, 'fuTv7tYSSzmbq187M-1VqK:APA91bFH1yz-hHupmNLVa4TljVR8Q7mlFZ_IlQp75CsqYL5hd_v8rJBI8NWEchwTerc2bLQmkAmQIgJmP8cOXbM0ZNXTvofVwt2YnGH5wEY81fiTUttSO14S3H70tixoSZE2r2-BoOJL'),
(2, 'Fedi', 'mersnifedy@gmail.com', '$2b$10$ivaOvF4IoGNW4AoOom06L.Z0SGKctHrTn2ay84ZvSZyluJ3xqOaSm', 2, NULL, 3, 'fS1B1jSoQWeRohCz0tE6i2:APA91bEcLBtyw0bbArJmuBuo47l_G82VQuq0_vWINm7VNFuHPwNHvJkoyRhLYpf9583KRCcuhi8s-irwW2jGz_gp3CcUCwjHeT6nnb2lMdj5Du2ebdY0XkxzJgC7lGDnftRBOEuv4Y50'),
